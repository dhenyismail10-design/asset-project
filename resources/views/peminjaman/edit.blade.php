...
<div class="container-fluid p-0" style="max-width: 700px; margin: 0 auto;">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            <h2 class="fw-800 text-slate-900 mb-1" style="font-weight: 800; letter-spacing: -0.5px;">Edit Peminjaman Aset</h2>
            <p class="text-muted mb-0 font-medium">Perbarui informasi data peminjaman di bawah ini.</p>
        </div>
            ...
            <form action="{{ route('peminjaman.update', $peminjaman->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" name="peminjam" id="peminjam" class="form-control border-start-0 @error('peminjam') is-invalid @enderror" style="border-radius: 0 10px 10px 0;" placeholder="Masukkan nama peminjam" value="{{ old('peminjam', $peminjaman->peminjam) }}" required>
                    </div>
                    @error('peminjam')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                    @enderror
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" name="tanggal_pinjam" id="tanggal_pinjam" class="form-control @error('tanggal_pinjam') is-invalid @enderror" value="{{ old('tanggal_pinjam', $peminjaman->tanggal_pinjam) }}" required>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" name="tanggal_kembali" id="tanggal_kembali" class="form-control @error('tanggal_kembali') is-invalid @enderror" value="{{ old('tanggal_kembali', $peminjaman->tanggal_kembali) }}">
                ...
                <div class="mb-4">
                    ...
                    <select name="kondisi_awal" id="kondisi_awal" class="form-select @error('kondisi_awal') is-invalid @enderror" required>
                        <option value="Baik" {{ old('kondisi_awal', $peminjaman->kondisi_awal) == 'Baik' ? 'selected' : '' }}>Baik</option>
                        <option value="Rusak Ringan" {{ old('kondisi_awal', $peminjaman->kondisi_awal) == 'Rusak Ringan' ? 'selected' : '' }}>Rusak Ringan</option>
                        <option value="Rusak Berat" {{ old('kondisi_awal', $peminjaman->kondisi_awal) == 'Rusak Berat' ? 'selected' : '' }}>Rusak Berat</option>
                    </select>
                    @error('kondisi_awal')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                    @enderror
                ...
                <div class="d-flex justify-content-end gap-2">
                    ...
                    <button type="submit" class="btn btn-indigo text-white fw-semibold px-4 py-2 rounded-3 shadow-sm" style="background: #4f46e5; border: none;">
                        <i class="bi bi-check-lg me-1"></i> Perbarui Peminjaman
                    </button>
                </div>
            </form>
